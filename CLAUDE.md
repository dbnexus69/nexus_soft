...
- **Person data is validated in one place per side.** Names, document number by document type (CC/TI/CE/NIT numeric, PA alphanumeric), phone and birth date follow `backend/src/utils/datosPersona.js` (authoritative; `schemas/clients.schema.js` applies it and the controller passes `req.validatedBody`, already normalized) and are mirrored by hand in `frontend/src/utils/datosPersona.ts` — change both together. A 422 carries `error.details: [{ field, message }]` and the modal paints each one next to its input, so hooks must rethrow the original error, never wrap it in `new Error(msg)` (spec 003).
- **A sale is created whole.** The wizard builds the draft in the browser (`localStorage`, keyed by empresa and user) and `POST /sales` writes the sale and all its products in one `transaccion`; each product list (`ticketData`, `hotelData`…) is validated per item by `schemas/products.schema.js` (422 names the field, e.g. `hotelData.0.hotelType`). There is no adding, editing or deleting products of an existing sale — those endpoints were unused and were removed (spec 002, T8). Files never travel in the sale JSON: the 201 returns `products: [{ category, index, detalleId }]`, and the browser then uploads each voucher with `PUT /sales/:saleId/products/:detalleId/voucher` (multipart `file`). Unknown `/api` routes answer 404 `ROUTE_NOT_FOUND` in the API format.
- **Flights use Bogotá days, and direction is inferred.** `dateFrom`/`dateTo` on `/flights` and `/flights/checkins` are `YYYY-MM-DD` days in Bogotá, both inclusive (`rangoDeDias` → `[desde, hasta)`); the browser sends the month as two strings, never `toISOString()`. Ida/regreso is not stored: in a `round_trip` the legs are cut at the largest gap between arrival and next departure. Ticket airports and baggage plans (`"<aerolínea> - <tarifa>"` or id) are validated before the transaction in `sales.service.js` (`_validarTiquetes`) — unknown → 422 with the field, there is no `UNK` airport. `recalcularProducto` locks the product row (`FOR UPDATE`). Plan flights cannot be cancelled (no columns) (spec 004; `docs/designs/vuelos-fechas-de-bogota-y-direccion.md`).
...
- `docs/specs/` — spec-driven work, one folder per spec with `spec.md` (what and how it is checked), `plan.md` (how) and `tasks.md` (done/pending, with what was verified): `001-multi-tenant` (the build) and `002-estabilizacion-multi-tenant` (what broke once it ran, and what is still pending) and `003-validacion-de-datos-de-personas` (field rules, clients first) and `004-vuelos-y-checkin` (flights calendar and check-in: Bogotá-day date ranges, ida/regreso inferred on read, ticket airports and baggage plans validated before the transaction). Update the `tasks.md` of the spec you are working under; new work gets a new numbered folder.
